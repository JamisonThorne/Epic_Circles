...
def main(): 
    Epic_Circles = Decartes_Theorem()
    Epic_Circles.Circle_Hub()

class Decartes_Theorem:
    def __init__(self):
        self.initial_circle_radius = 2                      #   Initial starting circle radius
        self.graph_bounds = self.initial_circle_radius*1.5    #   Initial Graph bounds for plotting circles
        self.Circle_Statistics = []                         #   Array containing circles with their complex number follow by associated curvature        
        
    def Circle_Hub(self):
        #####################################################################################################################
        #   Background information on Decartes Theorem, Appolonian Gaskets and complex maths showcased in this script can be found here:
        #       https://en.wikipedia.org/wiki/Descartes%27_theorem
        #       https://math.umd.edu/~immortal/MATH431/book/ch_complex.pdf
        #       https://sites.math.washington.edu//~julia/teaching/445_Spring2013/DescartesAndTheApollonianGasket%20(1).pdf
        #####################################################################################################################
        #   Initial declaration of the first three circles is needed, Decartes Theorem will take care of the rest
        self.Circle_Statistics.append([complex(0,0),(self.initial_circle_radius)])                                     #  Define first circle
        self.Circle_Statistics.append([complex(0,(self.initial_circle_radius/2)),self.initial_circle_radius/2])    #  Define second circle
        self.Circle_Statistics.append([complex(0,(self.initial_circle_radius/2-self.initial_circle_radius)),self.initial_circle_radius/2])  #  Define third circle
        ###############################################
        # self.Recursive_Descartes_Theorem()  #   Using the above three circles, recursively find all other needed circles
        self.Circle_Plots()
        
    def Circle_Plots(self):
        # need to implement for loop that iterates through self.Circle_Statistics, come back to this later
        # Figure Generation
        fig, ax = plt.subplots() # note we must use plt.subplots, not plt.subplot
        ax.cla() # clear things for fresh plot
        # change default range so that new circles will work
        ax.set_xlim((-1*self.graph_bounds, self.graph_bounds))
        ax.set_ylim((-1*self.graph_bounds, self.graph_bounds))
        for i in range(0,len(self.Circle_Statistics)):
            circle = plt.Circle((self.Circle_Statistics[i][0].real, self.Circle_Statistics[i][0].imag), -1*self.Circle_Statistics[i][1], color='r',fill=False)
            ax.add_patch(circle)
        plt.show()
